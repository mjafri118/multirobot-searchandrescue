'''
Harvard CS 286 Spring 2022 - Baseline
'''
...
class Env:
    ...
    def safe_wander(self, flock=False):
        ...
        if flock:
            ...
            for bot in self.bots:
                x_temp,y_temp = self._move_random_step()
                self.move_bot(bot, [x_temp,y_temp])
...
if __name__ == "__main__":
    ...
    time.sleep(3)
    bot2 = Robot(9,9,'t3') #Target
    bot2.sim_bot.rotate(60,180,0)
    bot1.sim_bot.rotate(30,360,0)

    # env1.display_grid()
    (rand_x, rand_y) = env1._move_random_step()
    while 9+rand_x != 9 and 1+rand_y != 9:
        bot1.sim_bot.rotate(90,45,0)
        bot1.sim_bot.go_to(9+rand_x,1+rand_y)
        time.sleep(1)
 
    print('Step 0 ----------------Hunting: Based on Random walk, until hit the target.')
