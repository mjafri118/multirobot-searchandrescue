...
class RobotSLAM_Nav:
	...
	#Use the variables self.current_position and self.current_ori to store the position and orientation values.
        ...
        self.current_position.z = z_position
        self.current_ori.x = orientation_x
        self.current_ori.y = orientation_y
        self.current_ori.z = orientation_z
        self.current_ori.w = orientation_w
    ...
    def move_along_direction(self):
        ...
        while not rospy.is_shutdown():    
            if(self.gotAOA):
                ...
                theta = self.move_direction
                x = np.cos(np.deg2rad(theta)) + self.current_position.x
                y = np.sin(np.deg2rad(theta)) + self.current_position.y
                if theta > 90:
                    x = -abs(x)
                elif theta < -90:
                    x = -abs(x)
                ...
                self.goal.target_pose.pose.orientation.x = self.current_ori.x #[0]
                self.goal.target_pose.pose.orientation.y = self.current_ori.y #[1]
                self.goal.target_pose.pose.orientation.z = self.current_ori.z #[2]
                self.goal.target_pose.pose.orientation.w = self.current_ori.w #[3]
