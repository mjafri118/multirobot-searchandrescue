...
def get_patrolling_locations(SEARCHER_CONFIGS, CURRENT_SEARCHER_IDX):
    ...
    for idx in range(158):
        for idy in range(85):
            map[idx,idy] = random.choices(
                population = [-1,0,25,100],
                weights= [.65, .25, .1])
...
def neopolitan(searcher_locations, map):
    ...
    patrolling_goal_locations = [(-1,-1,-1)] * len(searcher_locations) 
    ...
    for searcher_idx in range(len(searcher_locations)):
        ...
        patrolling_goal_locations[searcher_idx] = room_frame_to_robot_frame(layers_used * 1.5 * layer_width, map_height/2)
...
def dontMove(searcher_locations, map):
    return [room_frame_to_robot_frame(searcher_location) for searcher_location in searcher_locations]
...
def robot_frame_to_room_frame(robot_frame):
    return (
        round((robot_frame.x + 6.85 - drop_point_offset_x)/.05), 
        round((robot_frame.y + 1 + drop_point_offset_y)/.05),
    )

def room_frame_to_robot_frame(room_frame):
    return (
        room_frame.x * .05 + drop_point_offset_x - 6.85,
        room_frame.y * .05 - drop_point_offset_y - 1,
    )    